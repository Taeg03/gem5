...
# CLI Arguments:
# 1: core_type ("MinorCPU" or "DerivO3CPU")
# 2: issue_width (2, 4, 8)
# 3: rob_size (32, 64, 128 for O3; "inactive" or 0 for Minor)
# 4: l1d_mshrs (2, 4, 8)
# 5: l1d_size ("16kB", "32kB", "64kB")
# 6: l1d_assoc (2, 4, 8)
# 7: l2_size ("512kB", "1MB", "2MB")
# 8: workload_mode ("compute", "latency", "concurrency")

core_type = sys.argv[1]
issue_width = int(sys.argv[2])
rob_size_str = sys.argv[3]
rob_size = int(rob_size_str) if rob_size_str not in ["inactive", "None", "0"] else 0
l1d_mshrs = int(sys.argv[4])
l1d_size = sys.argv[5]
l1d_assoc = int(sys.argv[6])
l2_size = sys.argv[7]
...
# 1. Parameterized CPU Core
if core_type == "MinorCPU":
    system.cpu = X86MinorCPU()
    # Explicit mapping of pipeline stage widths for MinorCPU
    system.cpu.fetch1FetchLimit = issue_width
    system.cpu.decodeInputWidth = issue_width
    system.cpu.executeInputWidth = issue_width
    system.cpu.executeIssueLimit = issue_width
    system.cpu.executeCommitLimit = issue_width
    # Note: rob_size, rename, and out-of-order queues do NOT exist in MinorCPU
elif core_type == "DerivO3CPU":
